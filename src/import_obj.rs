...
fn handle_f(obj: &mut WFObjectState, line: SplitWhitespace) -> Option<()> {
    ...
    obj.face_list.extend(line.map(|f| {
        FaceMarker::Def(FaceDef::new(f).sanitize(&obj.vert_list, &obj.normal_list, &obj.tex_list))
    }));
    obj.face_list.push(FaceMarker::End);

    Some(())
}

fn handle_o(obj: &mut WFObjectState, line: SplitWhitespace) -> Option<()> {
    Some(())
}
...
fn get_concave_vertex(indicies: &[u32], vs: &[VertexTexture]) -> [u32; 4] {
    ...
    return [indicies[0], indicies[1], indicies[2], indicies[3]];
}
...
fn pack_wf_state(obj: &WFObjectState) -> (Vec<VertexTexture>, Vec<[u32; 3]>) {
